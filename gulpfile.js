...
function rollup(done) {
	...
	// if (typeof cb === 'string') { // abort if blacklisted
	// 	let file = cb.replace(/\\/g, "/");
...
gulp.task("build", gulp.series(gulp.parallel(['workers', 'shaders', "icons_viewer"]), output, rollup));

gulp.task('watch', gulp.series("build", function rollup(done) {
	//gulp.run("build");
	...
	gulp.watch(watchlist, {ignored: 'resources/icons/index.html'}, gulp.series('build'));
	// done()
}))
